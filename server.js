...
app.post('/signup', (req, res) => {
  ...
  db.run(
    ...
    [username, password, '', '/images/user.png'],
    function (err) {
      if (err) {
      ...
      req.session.user = {
        id: this.lastID, username, password, bio: '', profilePicture: 'Default Profile Pic'
      };
      res.redirect('/home');
    }
  );
...
app.post('/profile', requireLogin, (req, res) => {
  ...
  db.run(
    ...
    [bio || req.session.user.bio, profilePicture || req.session.user.profilePicture, username || req.session.user.username, req.session.user.id],
    (err) => {
      if (err) console.error(err.message);
...
app.get('/messages/:userId', requireLogin, (req, res) => {
  ...
  db.get('SELECT id, username FROM users WHERE id = ?', [userId], (err, recipient) => {
    ...
    db.all(
      `SELECT m.*, u.username as fromUsername, u.profilePicture as fromPicture 
            FROM messages m 
